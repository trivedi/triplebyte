...
class Request:
	...
	def do_get(self):
		...
		return header + self.content # extra header needed to separate from body per HTTP standards (RFC 2616)
	...
	def do_head(self):
		...
		'''
		crlf = '\r\n'
		self.content = self.get_content() # save the content of in case GET is next
		...
		header += 'Content-Length: %d' % len(self.content) + crlf
	...
	def get_content(self):
		'''
		Return content of @self.filename
		'''
		# Serve templated files in case there isn't an OK HTTP response
		if self.status != 200:
			self.filename = util.create_path('/resp_files', '/'+str(self.status)+'.html')
